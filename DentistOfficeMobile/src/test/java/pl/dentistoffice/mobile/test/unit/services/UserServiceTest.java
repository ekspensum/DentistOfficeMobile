...
class UserServiceTest {
	...
	void testLoginPatient() {
	    ...
	    responseEntity = new ResponseEntity<Patient>(patient, headers, HttpStatus.UNAUTHORIZED);
		when(restTemplate.postForEntity(URL_LOGIN, loginData, Patient.class)).thenReturn(responseEntity);
	    assertThrows(HttpClientErrorException.class, () -> userService.loginPatient(username, password, httpSession, model));
	}
	...
	void testLogoutPatient() {
		...
		responseEntity = new ResponseEntity<Boolean>(HttpStatus.NOT_FOUND);
		when(restTemplate.exchange(URL_LOGOUT, HttpMethod.POST, requestEntity, Boolean.class)).thenReturn(responseEntity);
		assertThrows(HttpClientErrorException.class, () -> userService.logoutPatient(patientId, token));
	}
	...
	void testGetAllDoctors() {
		...
		responseEntity = new ResponseEntity<DoctorListWrapper>(HttpStatus.NOT_FOUND);
		when(restTemplate.getForEntity(URL_DOCTORS, DoctorListWrapper.class)).thenReturn(responseEntity);
		assertThrows(HttpClientErrorException.class, () -> userService.getAllDoctors());
	}
	...
	void testRegisterPatient() throws NoSuchFieldException, SecurityException {
		...
		when(restTemplate.exchange(URL_REGISTER_PATIENT, HttpMethod.POST, requestEntity, String.class)).thenReturn(responseEntity);
		assertThrows(HttpClientErrorException.class, () -> userService.registerPatient(patient));
	}
	...
	void testEditPatient() {
		...
		ResponseEntity<String> responseEntity = new ResponseEntity<String>("true", header, HttpStatus.OK);
		when(restTemplate.exchange(URL_REGISTER_PATIENT, HttpMethod.PUT, requestEntity, String.class)).thenReturn(responseEntity);
		userService.editPatient(token, patient);
		assertTrue(!responseEntity.getStatusCode().isError());
		assertEquals("true", responseEntity.getBody());
		responseEntity = new ResponseEntity<String>("error", header, HttpStatus.OK);
		when(restTemplate.exchange(URL_EDIT_PATIENT, HttpMethod.PUT, requestEntity, String.class)).thenReturn(responseEntity);
		...
		responseEntity = new ResponseEntity<String>(header, HttpStatus.UNAUTHORIZED);
		when(restTemplate.exchange(URL_REGISTER_PATIENT, HttpMethod.PUT, requestEntity, String.class)).thenReturn(responseEntity);
		assertThrows(HttpClientErrorException.class, () -> userService.editPatient(token, patient));
	}
